...
import { useQuizAnimations } from "../../hooks/useGSAPAnimations"
...
export default function QuizHomepage() {
  ...
  const totalQuizzesRef = useRef<HTMLParagraphElement>(null)
  const premiumQuizzesRef = useRef<HTMLParagraphElement>(null)
  const filteredCountRef = useRef<HTMLParagraphElement>(null)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setTimeout(() => {
          if (totalQuizzesRef.current) {
            animateCounter(totalQuizzesRef.current, 0, quizzesData.length)
          }
          if (premiumQuizzesRef.current) {
            const premiumCount = quizzesData.filter(q => q.is_premium).length
            animateCounter(premiumQuizzesRef.current, 0, premiumCount)
          }
          if (filteredCountRef.current) {
            animateCounter(filteredCountRef.current, 0, quizzesData.length)
          }
        }, 800)
  ...
  useEffect(() => {
    ...
    // Category filter (if we had categories in the backend)
    ...
    if (quizzes.length > 0) {
      ...
      if (filteredCountRef.current) {
        const currentCount = parseInt(filteredCountRef.current.textContent || "0")
        animateCounter(filteredCountRef.current, currentCount, filtered.length)
      }
    }
  }, [searchTerm, selectedCategory, selectedDifficulty, showPremiumOnly, quizzes, animateFilterChange, animateCounter])
    ...
    <div ref={containerRef} className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div ref={headerRef} className="flex items-center justify-between mb-4">
            ...
            {userUtils.hasPremiumAccess(user) ? (
              <Button 
                onClick={(e) => handleButtonClick(e, () => navigate('/create'))} 
              ...
              <Button 
                onClick={(e) => handleButtonClick(e, () => navigate('/upgrade'))} 
                className="bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white transition-all duration-300"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <Card ref={(el) => setStatsRef(el, 0)} className="hover:scale-105 transition-all duration-300">
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  <BookOpen className="w-5 h-5 text-blue-500" />
                  <div>
                    <p ref={totalQuizzesRef} className="text-2xl font-bold">0</p>
            ...
            <Card ref={(el) => setStatsRef(el, 1)} className="hover:scale-105 transition-all duration-300">
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  <Crown className="w-5 h-5 text-yellow-500" />
                  <div>
                    <p ref={premiumQuizzesRef} className="text-2xl font-bold">0</p>
            ...
            <Card ref={(el) => setStatsRef(el, 2)} className="hover:scale-105 transition-all duration-300">
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  <Users className="w-5 h-5 text-green-500" />
                  <div>
                    <p ref={filteredCountRef} className="text-2xl font-bold">0</p>
        ...
        {filteredQuizzes.length === 0 ? (
          <Card className="text-center p-8 opacity-0" style={{ animation: 'fadeInUp 0.6s ease-out forwards' }}>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <Button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        handleButtonClick(e, () => handleQuizClick(quiz._id))
                      }}
                    >
      ...
      {/* Add custom CSS for fallback animations */}
      <style jsx>{`
        @keyframes fadeInUp {
          from {
            opacity: 0;
